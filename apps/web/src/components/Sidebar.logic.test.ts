...
describe("resolveThreadStatusPill", () => {
  ...
  it("shows pending approval before all other statuses", () => {
    ...
    ).toMatchObject({ label: "pendingApproval", pulse: false });
  });
  ...
  it("shows awaiting input when plan mode is blocked on user answers", () => {
    ...
    ).toMatchObject({ label: "awaitingInput", pulse: false });
  });
  ...
  it("shows plan ready when a settled plan turn has a proposed plan ready for follow-up", () => {
    ...
    ).toMatchObject({ label: "planReady", pulse: false });
  });
...
describe("resolveProjectStatusIndicator", () => {
  ...
  it("surfaces the highest-priority actionable state across project threads", () => {
    ...
    ).toMatchObject({ label: "pendingApproval", dotClass: "bg-amber-500" });
  });
  ...
  it("prefers plan-ready over completed when no stronger action is needed", () => {
    ...
    ).toMatchObject({ label: "planReady", dotClass: "bg-violet-500" });
  });
});
...
describe("getVisibleThreadsForProject", () => {
  ...
  it("returns the preview threads unchanged when the active thread is already visible", () => {
    const threads = Array.from({ length: 8 }, (_, index) =>
      makeThread({
        id: ThreadId.makeUnsafe(`thread-${index + 1}`),
      }),
...
describe("sortProjectsForSidebar", () => {
  it("sorts projects by the most recent thread update across their threads", () => {
    const projects = [
  ...
  it("uses project creation timestamps when sorting by createdAt", () => {
