...
function getUiCopy(language: AppLanguage) {
  ...
  return {
    ...
    rateLimits: {
      title: it ? "Limiti OpenAI" : "OpenAI limits",
      fiveHourWindow: it ? "Finestra 5 ore" : "5-hour window",
      weeklyWindow: it ? "Finestra settimanale" : "Weekly window",
      resetIn: (value: string) => (it ? `Reset tra ${value}` : `Resets in ${value}`),
      ariaLabel: (value: string) =>
        it ? `${value} di limite 5 ore rimanente` : `${value} of 5-hour limit remaining`,
      plan: (value: string) => (it ? `Piano ${value}` : `${value} plan`),
    },
    compactComposer: {
